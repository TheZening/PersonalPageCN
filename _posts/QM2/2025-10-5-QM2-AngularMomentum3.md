...
我们现在来聊聊, 为什么我们之前的分解$$U_1 \oplus U_2$$, 是物理上更有意义的分解.
这就绕不开一个概念, 不变子空间.

> 设$$V$$是一个线性空间, $$T: V \to V$$是$$V$$上的一个线性变换.
> 如果$$W$$是$$V$$的一个子空间, 且$$\forall \ket{w} \in W, T\ket{w} \in W$$,
> 那么$$W$$是$$T$$的不变子空间.
...
因为$$\forall \ket{w} \in W, \ket{w} = (x,0)$$, 那么$$T\ket{w} = (\cos\theta x, \sin\theta x)$$, 不在$$W$$中.
$$W = \{(0,y)|y\in\mathbb{R}\}$$也不是$$T$$的不变子空间.
因为$$\forall \ket{w} \in W, \ket{w} = (0,y)$$, 那么$$T\ket{w} = (-\sin\theta y, \cos\theta y)$$, 不在$$W$$中.
但是$$V$$本身是$$T$$的不变子空间.
因为$$\forall \ket{v} \in V, T\ket{v} \in V$$.
$$\{\ket{0}\}$$也是$$T$$的不变子空间.
因为$$\forall \ket{0} \in \{\ket{0}\}, T\ket{0} = \ket{0} \in \{\ket{0}\}$$.

> 这样我们就感受到两一个事实: 任何线性空间$$V$$和线性变换$$T: V \to V$$, $$V$$本身和$$\{\ket{0}\}$$都是$$T$$的不变子空间.
> 这两个子空间是平凡的, 因为大家谁都有.
...
因为$$\forall \ket{w} \in W, \ket{w} = (x,y,0)$$, 那么$$T\ket{w} = (\cos\theta x - \sin\theta y, \sin\theta x + \cos\theta y, 0)$$, 仍然在$$W$$中.
$$U = \{(0,0,z)|z\in\mathbb{R}\}$$也是$$T$$的不变子空间.
因为$$\forall \ket{u} \in U, \ket{u} = (0,0,z)$$, 那么$$T\ket{u} = (0,0,z)$$, 仍然在$$U$$中.
这两, 除了两个平凡子空间, 这是我们能找到的唯一两个不变子空间, 而且我们也心安理得地把$$V$$拆成了$$W \oplus U$$.

> 这就是我们想要的分解, 因为$$W$$和$$U$$都是$$T$$的不变子空间!
> 于是, 我们发现, 能否把一个线性空间$$V$$拆成若干个不变子空间的直和, 主要取决于针对什么算符$$T$$.

> 我们为什么选择用不变子空间拆角动量算符?
> 1. 我们要知道, 我们的本征态必须同时是$$J^2$$和$$J_z$$的本征态.
> 2. 我们也知道 $$J_\pm$$ 是 $$J_z$$ 的升降算符, 不影响 $$J^2$$ 的本征值.
> 3. 这就意味着, 如果我们找到一个$$J^2$$和$$J_z$$的共同不变子空间, 那么我们就能在这个子空间里找到一组$$J^2$$和$$J_z$$的共同本征态.
> 4. 自然而然的, 我们就想把整个空间拆成若干个$$J^2$$和$$J_z$$的共同不变子空间的直和.
> 
> 如果我们用$$D$$和$$O$$拆分, 那么$$D$$和$$O$$都可能被$$J_z$$和$$J^2$$作用后, 跑到对方的空间里去.
