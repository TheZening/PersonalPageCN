...
> (Fermionic creation operator): $$\hat{c}_i^\dagger$$ 的作用是将模式 $$i$$ 上的粒子数增加1, 前提是该模式上没有粒子:
> $$
> \begin{equation}
> \hat{c}_i^\dagger \ket{n_1, n_2, \ldots, n_i, \ldots } = \sqrt{1 - n_i} \ket{n_1, n_2, \ldots, n_i + 1, \ldots }
> \end{equation}
> $$

> (Fermionic annihilation operator): $$\hat{c}_i$$ 的作用是将模式 $$i$$ 上的粒子数减少1:
> $$
> \begin{equation}
> \hat{c}_i \ket{n_1, n_2, \ldots, n_i, \ldots } = \sqrt{n_i} \ket{n_1, n_2, \ldots, n_i - 1, \ldots }
> \end{equation}
> $$

通过类似的计算, 我们可以得到费米子创建和湮灭算符的反对易关系:
...
这个算符的本征值也是模式 $$i$$ 上的粒子数 $$n_i$$, 但由于费米子只能占据0或1个粒子, 因此 $$n_i$$ 只能是0或1, 稍微简单一些.
总结费米子系统的反对易关系:

1. $$\{\hat{c}_i, \hat{c}_j^\dagger\} = \delta_{ij}$$
2. $$\{\hat{c}_i, \hat{c}_j\} = 0$$
3. $$\{\hat{c}_i^\dagger, \hat{c}_j^\dagger\} = 0$$
4. $$[\hat{N}_i, \hat{c}_j^\dagger] = -\delta_{ij} \hat{c}_j^\dagger$$
5. $$[\hat{N}_i, \hat{c}_j] = \delta_{ij} \hat{c}_j$$

这就是Canonical anticommutation relations, (CAR)代数规则, 也被称为Clifford代数, 或者是Grassmann外代数.