var store = [{
        ...
        "title": "QM2 量子信息基础1",
        "excerpt":"量子信息科学: 研究如何利用量子力学的基本原理来处理和传输信息的学科. 量子计算, 量子通信和量子密码学等领域都是量子信息科学的重要应用. 量子信息学的底层逻辑是: 当我们试图在一个允许干扰的物理世界中描述信息的时候, 传统的概率论失效了, 我们需要一套基于 \\(L^2\\) 范数的广义概率理论. 经典概率论的失效 在经典概率论中, 我们可以通过概率分布来描述一个系统的状态. 例如, 一个经典比特可以处于状态 0 或 1, 并且我们可以用概率分布 \\(P_0\\) 和 \\(P_1\\) 来描述它处于这两种状态的概率: \\[\\begin{equation} \\bm{P} = (P_0, P_1), \\quad P_0 + P_1 = 1, \\quad P_0, P_1 \\geq 0 \\end{equation}\\] 这就是 \\(L^1\\) 范数的概率分布. (\\(L^1\\) 范数): 这是指向量中各元素绝对值之和的范数, 在概率论中用来表示概率分布的规范化条件. 然而, 在量子力学中, 系统的状态不能简单地用经典概率分布来描述....","categories": ["QM2"],
        "tags": [],
        "url": "/PersonalPageCN/qm2/QM2-QuantumFoundation/",
        "teaser": null
      }]
