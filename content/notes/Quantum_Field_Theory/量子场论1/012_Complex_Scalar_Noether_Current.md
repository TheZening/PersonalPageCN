...
诺特定理保证在满足运动方程的场构型上有 $\partial_\mu j^\mu=0$.
为了看清每一步, 直接对刚才的流求散度:
$$
...
\bm{j}=-\mathrm{i}[\phi^*\nabla\phi-(\nabla\phi^*)\phi]
$$
空间流的负号来自 $\partial^i=-\partial_i$.
这个连续性方程表示某一区域内诺特荷的变化只能通过边界通量发生, 不会在区域内部凭空产生或消失.


取固定空间区域 $V$ 并定义其中的荷:
$$
Q_V(t)=\int_Vj^0(t,\bm{x})\dd[3]{x}
$$
对时间求导并使用连续性方程:
$$
...
$$
J^\mu=qj^\mu \quad Q_{\mathrm{el}}=qQ
$$
把 $q$ 放在流中还是最后乘到荷上只是归一化约定.
为了避免把对称性生成元与具体单位混为一谈, 本文先保留无量纲 $Q$, 下一篇再显式写出 $Q_{\mathrm{el}}$.
...
注意 $\phi$ 的正则动量是 $\dot{\phi}^*$, 不是 $\dot{\phi}$.
...
若哈密顿量不显含时间, 任意相空间函数的演化为 $\dot{F}=\{F,H\}+\partial F/\partial t$.
由于 $Q$ 没有显式时间依赖且守恒:
$$
0=\dv{Q}{t}=\{Q,H\}
$$
