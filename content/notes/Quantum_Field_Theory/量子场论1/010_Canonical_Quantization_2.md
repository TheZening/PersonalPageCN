...
$$
[H,\pi(t,\bm{y})]=\mathrm{i}\int_V[\bm{\nabla}\phi(t,\bm{x})\cdot\bm{\nabla}_{\bm{x}}\delta_V^{(3)}(\bm{x}-\bm{y})+m^2\phi(t,\bm{x})\delta_V^{(3)}(\bm{x}-\bm{y})]\dd[3]{x}
$$
对第一项分部积分，并使用周期边界条件消去边界项：
$$
[H,\pi(t,\bm{y})]=-\mathrm{i}\bm{\nabla}^2\phi(t,\bm{y})+\mathrm{i}m^2\phi(t,\bm{y})
$$
所以第二条海森堡运动方程为：
$$
\partial_t\pi(t,\bm{y})=(\bm{\nabla}^2-m^2)\phi(t,\bm{y})
$$
再用 $\pi=\pdv*{\phi}{t}$ 消去共轭动量：
$$
(\partial_t^2-\bm{\nabla}^2+m^2)\phi=0
$$
...
$$
\hat{a}_{\bm{k}}\hat{a}_{\bm{k}}^{\dagger}=\hat{a}_{\bm{k}}^{\dagger}\hat{a}_{\bm{k}}+\mathbb{1}
$$
定义模占据数算符：
$$
...
从线性代数看，傅里叶变换把平移不变的二次哈密顿量对角化，每个动量模就是这个二次型的一个本征方向。
量子化后，总态空间由所有模的振子空间组合而成，哈密顿量则是各模数算符的加权和。
因此产生算符给出的不是某个时空纤维中的新向量，而是在量子态空间中沿一个单粒子本征方向增加一次占据。
...
定义真空 $\ket{0}$ 为被所有模湮灭算符消去的归一化态：
$$
\hat{a}_{\bm{k}}\ket{0}=0 \quad \braket{0}{0}=1
$$
因为 $\hat{N}_{\bm{k}}\ket{0}=0$，未减去常数的哈密顿量作用在真空上得到：
$$
H\ket{0}=E_0\ket{0} \quad E_0=\frac{1}{2}\sum_{\bm{k}}\omega_{\bm{k}}
$$
这是所有场模零点能的和。
它在无穷体积极限中变成：
$$
\frac{E_0}{V}=\frac{1}{2}\int\sqrt{\bm{k}^2+m^2}\frac{\dd[3]{k}}{(2\pi)^3}
$$
...
一个模激发态为 $\hat{a}_{\bm{p}}^{\dagger}\ket{0}$。
由于：
$$
...
可以把哈密顿量移过产生算符：
$$
H\hat{a}_{\bm{p}}^{\dagger}\ket{0}=\hat{a}_{\bm{p}}^{\dagger}H\ket{0}+[H,\hat{a}_{\bm{p}}^{\dagger}]\ket{0}=(E_0+\omega_{\bm{p}})\hat{a}_{\bm{p}}^{\dagger}\ket{0}
$$
...
期望值的定义与普通量子力学完全相同。
对归一化态 $\ket{\Psi}$ 和可观测量 $\mathcal{O}$：
$$
\expval{\mathcal{O}}_{\Psi}=\mel*{\Psi}{\mathcal{O}}{\Psi}
$$
如果 $\ket{\Psi}$ 是 $\mathcal{O}$ 的本征态，每次理想测量都得到同一本征值。
如果它是多个本征态的叠加，期望值是多次同样制备和测量的统计平均，而不是单次必然得到的结果。
不确定度由方差给出：
$$
(\Delta\mathcal{O})^2=\expval{\mathcal{O}^2}_{\Psi}-\expval{\mathcal{O}}_{\Psi}^2
$$
对任意占据数基矢，正规序哈密顿量的方差为零，因为该基矢本身就是能量本征态。
真空期望值是特别重要的情形，通常简写为 $\expval{\mathcal{O}}_0=\mel*{0}{\mathcal{O}}{0}$。
...
因此 $\hat{a}_{\bm{p}}^{\dagger}\ket{0}$ 也是总动量本征态，本征值为 $\bm{p}$。
能量和动量两项证据同时确立了粒子解释。
...
对任意态 $\ket{\Psi}$：
$$
\expval{\hat{N}_{\bm{k}}}_{\Psi}=\expval{\hat{a}_{\bm{k}}^{\dagger}\hat{a}_{\bm{k}}}_{\Psi}=\|\hat{a}_{\bm{k}}\ket{\Psi}\|^2\geq0
$$
...
尤其要注意，$:\hat{a}\hat{a}^{\dagger}:$ 与原来的 $\hat{a}\hat{a}^{\dagger}$ 并不相等，两者相差单位算符 $\mathbb{1}$。
