...
$$
这正是无质量粒子的色散关系。
因此零质量是麦克斯韦规范动力学的结果，而不是在求平面波时另行附加的色散关系。
...
对固定的非零光状四动量 $k$，洛伦兹条件先选出与 $k$ 正交的子空间 $k^\perp$，规范等价再把其中沿 $k$ 的一维子空间识别为零。
所以真正的物理偏振空间是二维商空间 $k^\perp/\operatorname{span}\{k\}$。
当 $k$ 在正能光锥上变化时，每个动量上方都有这样一根二维偏振纤维，它们共同组成质量壳上的物理偏振丛。
$\varepsilon_1(k)$ 与 $\varepsilon_2(k)$ 不是两种额外物理场，而是这根二维纤维中选定的一组局部基底。


这里还藏着 Zee 所强调的一个关键点：光子找不到静止系。
有质量粒子的标准动量可取为 $(m,0,0,0)$，保持它不变的洛伦兹变换给出通常的三维旋转；光状标准动量只能取为 $(\omega,0,0,\omega)$，其小群中除绕传播轴的旋转外，还有把偏振改变为 $\varepsilon^\mu+c k^\mu$ 的变换。
后一种变化正是规范变化，所以对无质量自旋为 $1$ 的场，洛伦兹变换与规范等价不是两套互不相干的结构。
若要同时保留洛伦兹协变性和两个螺旋度，就必须把相差 $k^\mu$ 的偏振向量视为同一个物理偏振。
...
无质量粒子没有静止系；对给定动量，最自然的标签是自旋在动量方向上的投影，也就是螺旋度。
对无质量粒子，螺旋度在正规正时洛伦兹变换下不变，而空间反演会把 $h=+1$ 与 $h=-1$ 互换。
...
$$
这个矩阵的具体分量依赖所选观察者标架，但不依赖横向平面内正交基的旋转。
不能简单写成 $-\eta^{\mu\nu}$，因为后者还包含时间方向和纵向方向。
...
从偏振丛角度看，参考向量 $n^\mu$ 帮助我们在每根商空间纤维中挑出具体横向代表，相当于选择局部标架或局部平凡化。
不同 $n^\mu$ 会改变偏振向量的分量表达，但不会改变商空间中的物理等价类。
因此偏振求和含参考向量不是洛伦兹协变性失败，而是“给商空间选代表元”不可避免地需要辅助选择。
