...
负频率部分为：
$$
A_{(-)}^\mu(x)=\sum_{\lambda=0}^3\int\frac{\varepsilon_\lambda^{\mu*}(\bm{k})\hat{a}_\lambda^\dagger(\bm{k})\mathrm{e}^{+\mathrm{i}k\cdot x}}{(2\pi)^3\sqrt{2\omega_{\bm{k}}}}\dd[3]{k}
$$
其中：
$$
\omega_{\bm{k}}=|\bm{k}|
...
所以：
$$
D_+^{\mu\nu}(z)=\expval{A_{(+)}^\mu(x)A_{(-)}^\nu(y)}_0
$$
把两个模展开代入：
$$
\begin{aligned}
D_+^{\mu\nu}(z)
&=\sum_{\lambda,\lambda'}\int\frac{\varepsilon_\lambda^\mu(\bm{k})\varepsilon_{\lambda'}^{\nu*}(\bm{q})\mathrm{e}^{-\mathrm{i}k\cdot x+\mathrm{i}q\cdot y}}{(2\pi)^6\sqrt{4\omega_{\bm{k}}\omega_{\bm{q}}}} \\
&\quad\times\expval{\hat{a}_\lambda(\bm{k})\hat{a}_{\lambda'}^\dagger(\bm{q})}_0\dd[3]{k}\dd[3]{q}
\end{aligned}
$$
真空矩阵元等于模对易子：
$$
\expval{\hat{a}_\lambda(\bm{k})\hat{a}_{\lambda'}^\dagger(\bm{q})}_0=s_\lambda(2\pi)^3\delta_{\lambda\lambda'}\delta^{(3)}(\bm{k}-\bm{q})
$$
完成 $\bm{q}$ 积分：
$$
D_+^{\mu\nu}(z)=\sum_{\lambda=0}^3s_\lambda\int\frac{\varepsilon_\lambda^\mu(\bm{k})\varepsilon_\lambda^{\nu*}(\bm{k})\mathrm{e}^{-\mathrm{i}k\cdot z}}{(2\pi)^3 2\omega_{\bm{k}}}\dd[3]{k}
$$
使用偏振完备关系：
$$
D_+^{\mu\nu}(z)=-\eta^{\mu\nu}D_{0,+}(z)
...
这里定义无质量标量怀特曼函数：
$$
D_{0,+}(z)=\int\frac{\mathrm{e}^{-\mathrm{i}k\cdot z}}{(2\pi)^3 2\omega_{\bm{k}}}\dd[3]{k}
$$
第二种场次序定义为：
$$
D_-^{\mu\nu}(z)=\expval{A^\nu(y)A^\mu(x)}_0
...
定义无质量泡利–若尔当分布：
$$
D_{0,+}(z)-D_{0,-}(z)=\mathrm{i}\Delta_0(z)
$$
于是：
$$
[A^\mu(x),A^\nu(y)]=-\mathrm{i}\eta^{\mu\nu}\Delta_0(x-y)
...
$$
z^2<0\Rightarrow\Delta_0(z)=0
$$
因此：
$$
(x-y)^2<0\Rightarrow[A^\mu(x),A^\nu(y)]=0
$$
...
定义：
$$
G_{\mu\nu,\rho\sigma,+}(z)=\expval{F_{\mu\nu}(x)F_{\rho\sigma}(y)}_0
$$
展开两个场强：
$$
G_{\mu\nu,\rho\sigma,+}=\pdv{}{x^\mu}\pdv{}{y^\rho}D_{\nu\sigma,+}-\pdv{}{x^\mu}\pdv{}{y^\sigma}D_{\nu\rho,+}-\pdv{}{x^\nu}\pdv{}{y^\rho}D_{\mu\sigma,+}+\pdv{}{x^\nu}\pdv{}{y^\sigma}D_{\mu\rho,+}
$$
其中：
$$
D_{\alpha\beta,+}(z)=-\eta_{\alpha\beta}D_{0,+}(z)
$$
又因为 $z=x-y$：
$$
\pdv{}{x^\mu}=\pdv{}{z^\mu}
