...
位置空间传播子已经被确定为 Dirac 算符带 Feynman 边界条件的逆核。
...
$$
D_F(z)=\int\frac{\mathrm{d}^4p}{(2\pi)^4}\frac{\mathrm{i}\mathrm{e}^{-\mathrm{i}p\cdot z}}{p^2-m^2+\mathrm{i}\epsilon}
$$
...
$$
S_F(z)=\int\frac{\mathrm{d}^4p}{(2\pi)^4}\frac{\mathrm{i}(\slashed p+m)}{p^2-m^2+\mathrm{i}\epsilon}\mathrm{e}^{-\mathrm{i}p\cdot z}
$$

定义动量空间传播子：

$$
S_F(z)=\int\frac{\mathrm{d}^4p}{(2\pi)^4}\widetilde S_F(p)\mathrm{e}^{-\mathrm{i}p\cdot z}
$$

于是：

$$
\widetilde S_F(p)=\frac{\mathrm{i}(\slashed p+m)}{p^2-m^2+\mathrm{i}\epsilon}
$$
...
$$
(\mathrm{i}\slashed\partial-m)S_F(z)=\int\frac{\mathrm{d}^4p}{(2\pi)^4}\frac{\mathrm{i}(\slashed p-m)(\slashed p+m)}{p^2-m^2+\mathrm{i}\epsilon}\mathrm{e}^{-\mathrm{i}p\cdot z}
$$

使用因式分解后：

$$
(\mathrm{i}\slashed\partial-m)S_F(z)=\int\frac{\mathrm{d}^4p}{(2\pi)^4}\frac{\mathrm{i}(p^2-m^2)}{p^2-m^2+\mathrm{i}\epsilon}\mathrm{e}^{-\mathrm{i}p\cdot z}
$$

在分布极限 $\epsilon\to0^+$ 中，分式趋于 $1$，所以：

$$
(\mathrm{i}\slashed\partial-m)S_F(z)=\mathrm{i}\int\frac{\mathrm{d}^4p}{(2\pi)^4}\mathrm{e}^{-\mathrm{i}p\cdot z}=\mathrm{i}\delta^{(4)}(z)
$$
...
$$
S_F(z)=\int\frac{\mathrm{d}^3p}{(2\pi)^3}\int\frac{\mathrm{d}p^0}{2\pi}\frac{\mathrm{i}N(p)\mathrm{e}^{-\mathrm{i}p^0z^0+\mathrm{i}\bm{p}\cdot\bm{z}}}{(p^0)^2-E_{\bm{p}}^2+\mathrm{i}\epsilon}
$$
...
$$
S_F(z)=\int\frac{\mathrm{d}^3p}{(2\pi)^3 2E_{\bm{p}}}(\slashed p+m)\mathrm{e}^{-\mathrm{i}p\cdot z}
$$
...
它在一般离壳动量处可逆，在质量壳上则出现二维核；传播子矩阵 $(\slashed p+m)/(p^2-m^2+\mathrm{i}\epsilon)$ 正是这个符号的分布逆。
因此分母标出矩阵失去可逆性的动量子流形，分子则把源旋量投向相应的解空间方向。
在正能质量壳上：

$$
\slashed p+m=\sum_su_s(p)\bar u_s(p)
$$
...
$$
\slashed p-m=\sum_sv_s(p)\bar v_s(p)
$$
