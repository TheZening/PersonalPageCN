...
其中无质量标量费曼传播子为：
$$
D_{F,0}(z)=\theta(z^0)D_{0,+}(z)+\theta(-z^0)D_{0,-}(z)
$$
为了避免把两个时间分支挤在同一条长公式中，分别记：
$$
D_{F,0,>}(z)=\theta(z^0)\int\frac{\mathrm{e}^{-\mathrm{i}k\cdot z}}{(2\pi)^3 2\omega_{\bm{k}}}\dd[3]{k}
$$
以及：
$$
D_{F,0,<}(z)=\theta(-z^0)\int\frac{\mathrm{e}^{+\mathrm{i}k\cdot z}}{(2\pi)^3 2\omega_{\bm{k}}}\dd[3]{k}
...
更明确地说，与电流耦合后 $y$ 点的协向量源 $J_\nu(y)$ 进入传播核，输出 $x$ 点的势分量 $A^\mu(x)$。
所以 $D_F^{\mu\nu}(x,y)$ 是一个连接两端纤维的分布核，一个指标负责源纤维，另一个指标负责响应纤维。
在闵可夫斯基时空选定全局标架后，这种双纤维映射被简写为 $-\eta^{\mu\nu}$ 乘标量核；若没有这种全局平直标架，就不能把不同点的张量指标未经说明地当成同一组分量。

---
## 接触项为什么带有相反于标量张量因子的符号


两种怀特曼函数作为分布都满足齐次波动方程：
$$
\Box D_\pm^{\mu\nu}(z)=0
$$
...
再求一次时间导数：
$$
\partial_0^2D_F^{\mu\nu}=\delta(z^0)(\partial_0D_+^{\mu\nu}-\partial_0D_-^{\mu\nu})+\theta(z^0)\partial_0^2D_+^{\mu\nu}+\theta(-z^0)\partial_0^2D_-^{\mu\nu}
$$
空间导数不作用在阶跃函数上。
利用两个时间分支的齐次方程，只剩：
$$
\Box D_F^{\mu\nu}(z)=\delta(z^0)[\partial_0D_+^{\mu\nu}(0,\bm{z})-\partial_0D_-^{\mu\nu}(0,\bm{z})]
$$
...
标量传播子满足：
$$
\Box D_{F,0}(z)=-\mathrm{i}\delta^{(4)}(z)
$$
再乘 $-\eta^{\mu\nu}$，正好得到光子传播子的接触项。
...
$$
D_{F,0}(z)=\int\frac{\mathrm{i}\mathrm{e}^{-\mathrm{i}k\cdot z}}{(2\pi)^4(k^2+\mathrm{i}\epsilon)}\dd[4]{k}
$$
因此费曼规范光子传播子为：
$$
D_F^{\mu\nu}(z)=\int\frac{-\mathrm{i}\eta^{\mu\nu}\mathrm{e}^{-\mathrm{i}k\cdot z}}{(2\pi)^4(k^2+\mathrm{i}\epsilon)}\dd[4]{k}
$$
采用傅里叶变换约定：
$$
D_F^{\mu\nu}(z)=\int\frac{\widetilde D_F^{\mu\nu}(k)\mathrm{e}^{-\mathrm{i}k\cdot z}}{(2\pi)^4}\dd[4]{k}
$$
动量空间传播子就是：
$$
...
得到：
$$
\Box D_F^{\mu\nu}(z)=\int\frac{\mathrm{i}\eta^{\mu\nu}k^2\mathrm{e}^{-\mathrm{i}k\cdot z}}{(2\pi)^4(k^2+\mathrm{i}\epsilon)}\dd[4]{k}
$$
取 $\epsilon\to0^+$ 的分布极限：
$$
\Box D_F^{\mu\nu}(z)=\mathrm{i}\eta^{\mu\nu}\int\frac{\mathrm{e}^{-\mathrm{i}k\cdot z}}{(2\pi)^4}\dd[4]{k}
$$
于是恢复：
$$
\Box D_F^{\mu\nu}(z)=\mathrm{i}\eta^{\mu\nu}\delta^{(4)}(z)
