...
在线性代数类比中，$\xi^\mu\pdv*{\phi_a}{x^\mu}$ 是因为改变取样位置而产生的差，$\Delta_a$ 是向量本身在搬运中的额外旋转，二者相减才得到同一基准点上的净分量变化。
...
$$
也可以完全改写成固定点变化。
对任意标量函数 $F$，总变分与定点变分满足 $\Delta F=\delta_0F/\epsilon+\xi^\mu\pdv*{F}{x^\mu}$。
因此：
$$
...
能量密度是 $T^{00}$。
由于 $\eta^{0\nu}\pdv*{\phi}{x^\nu}=\pdv*{\phi}{t}$ 且 $\eta^{00}=1$：
$$
T^{00}=(\partial_t\phi)^2-\mathcal{L}
$$
代入 $\mathcal{L}=[\pdv*{\phi}{t}]^2/2-\bm{\nabla}\phi\cdot\bm{\nabla}\phi/2-V$：
$$
...
可以用有限维线性代数理解方程不独立。
若干方程的左边组成向量 $F_A(q)$，存在非零系数 $c^A$ 使 $c^AF_A(q)=0$ 对任意 $q$ 都成立，那么这些方程之间有恒等关系，不能提供同样多份独立限制。
