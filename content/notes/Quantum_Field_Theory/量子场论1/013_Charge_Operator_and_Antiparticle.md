...
$$
J^\mu=q:j^\mu: \quad Q_{\mathrm{el}}=qQ
$$
下面先计算无量纲 $Q$, 最后只需乘上 $q$.
...
$$
所有不配对的基本等时对易子都为零.
这再次说明 $\phi$ 与 $\pi=\dot{\phi}^{\dagger}$ 是一对正则变量, $\phi^{\dagger}$ 与 $\pi^{\dagger}=\dot{\phi}$ 是另一对.
...
允许动量离散为 $\bm{k}=\frac{2\pi}{L}\bm{n}$, 其中 $\bm{n}\in\mathbb{Z}^3$.
定义正频率平面波:
$$
...
记 $f_{\bm{k}}=\frac{1}{\sqrt{2V\omega_{\bm{k}}}}$.
第一个乘积的 $a$ 类对角部分为:
$$
...
$$
利用 $[\hat{b}_{\bm{k}},\hat{b}_{\bm{k}}^{\dagger}]=1$ 改换第二项的次序:
$$
Q_{\text{形式}}=\sum_{\bm{k}}(\hat{N}_{a,\bm{k}}-\hat{N}_{b,\bm{k}}-1)
$$
最后的 $-1$ 对每个动量模都出现一次, 在无限模极限中成为发散的真空常数.
...
$$
前一种排序给每个模贡献 $-1$, 与它相反的排序给每个模贡献 $+1$, 对称平均后两个真空常数相消.
所以对称排序与正规序在这个自由荷算符上得到同一结果.
这也说明真正固定物理内容的是生成元对场的作用和真空中性的选择, 而不是未经规定的同点乘积.
物理电荷算符因此为:
$$
Q_{\mathrm{el}}=q\sum_{\bm{k}}(\hat{N}_{a,\bm{k}}-\hat{N}_{b,\bm{k}})
$$
连续极限只需把离散求和及克罗内克符号换成连续归一化:
$$
Q_{\mathrm{el}}=q\int[\hat{a}^{\dagger}(\bm{k})\hat{a}(\bm{k})-\hat{b}^{\dagger}(\bm{k})\hat{b}(\bm{k})]\dd[3]{k}
$$
...
$$
[Q_{\mathrm{el}},\hat{a}_{\bm{p}}^{\dagger}]=q\hat{a}_{\bm{p}}^{\dagger} \quad [Q_{\mathrm{el}},\hat{b}_{\bm{p}}^{\dagger}]=-q\hat{b}_{\bm{p}}^{\dagger}
$$
对湮灭算符则有:
$$
[Q_{\mathrm{el}},\hat{a}_{\bm{p}}]=-q\hat{a}_{\bm{p}} \quad [Q_{\mathrm{el}},\hat{b}_{\bm{p}}]=q\hat{b}_{\bm{p}}
$$
...
正规序保证真空满足:
$$
Q_{\mathrm{el}}|0\rangle=0
$$
定义归一化的一粒子态和一反粒子态:
$$
|\bm{p};a\rangle=\hat{a}_{\bm{p}}^{\dagger}|0\rangle \quad |\bm{p};b\rangle=\hat{b}_{\bm{p}}^{\dagger}|0\rangle
$$
把 $Q_{\mathrm{el}}$ 移过产生算符:
$$
Q_{\mathrm{el}}|\bm{p};a\rangle=\hat{a}_{\bm{p}}^{\dagger}Q_{\mathrm{el}}|0\rangle+[Q_{\mathrm{el}},\hat{a}_{\bm{p}}^{\dagger}]|0\rangle=q|\bm{p};a\rangle
$$
同理:
$$
Q_{\mathrm{el}}|\bm{p};b\rangle=-q|\bm{p};b\rangle
$$
...
$$
\hat{\mathcal{A}}^{\dagger}=\prod_{\bm{k}}\frac{(\hat{a}_{\bm{k}}^{\dagger})^{n^{(a)}_{\bm{k}}}}{\sqrt{n^{(a)}_{\bm{k}}!}}
$$
以及:
$$
\hat{\mathcal{B}}^{\dagger}=\prod_{\bm{k}}\frac{(\hat{b}_{\bm{k}}^{\dagger})^{n^{(b)}_{\bm{k}}}}{\sqrt{n^{(b)}_{\bm{k}}!}}
$$
相应的占据数基矢为:
$$
|\{n^{(a)}_{\bm{k}}\},\{n^{(b)}_{\bm{k}}\}\rangle=\hat{\mathcal{A}}^{\dagger}\hat{\mathcal{B}}^{\dagger}|0\rangle
$$
电荷算符的作用为:
$$
Q_{\mathrm{el}}|\{n^{(a)}\},\{n^{(b)}\}\rangle=q\sum_{\bm{k}}[n^{(a)}_{\bm{k}}-n^{(b)}_{\bm{k}}]|\{n^{(a)}\},\{n^{(b)}\}\rangle
$$
总激发数则为:
$$
N_{\mathrm{exc}}=\sum_{\bm{k}}(\hat{N}_{a,\bm{k}}+\hat{N}_{b,\bm{k}})
$$
电荷取两类占据数之差, 总激发数取两类占据数之和, 二者不能混为一谈.
在自由理论中二者都守恒.
在保持整体 $U(1)$ 对称性的相互作用理论中, $Q_{\mathrm{el}}$ 仍然守恒, 但总激发数一般不必守恒.
...
[H,Q_{\mathrm{el}}]=0
$$
海森堡方程于是给出 $\dot{Q}_{\mathrm{el}}=\mathrm{i}[H,Q_{\mathrm{el}}]=0$.
...
这里 $Q=\frac{Q_{\mathrm{el}}}{q}$ 是无量纲生成元.
定义幺正算符:
$$
...
$$
C\hat{a}_{\bm{k}}C^{-1}=\hat{b}_{\bm{k}} \quad C\phi(x)C^{-1}=\phi^{\dagger}(x)
$$
因此:
$$
CQ_{\mathrm{el}}C^{-1}=-Q_{\mathrm{el}}
$$
