...
在实势能 $V(\bm{x})$ 下：
$$
\mathrm{i}\partial_t\psi=\left(-\frac{\bm{\nabla}^2}{2M}+V\right)\psi
$$
复共轭方程为：
$$
-\mathrm{i}\partial_t\psi^*=\left(-\frac{\bm{\nabla}^2}{2M}+V\right)\psi^*
...
$$
\partial_t(\psi^*\psi)=\frac{\mathrm{i}}{2M}(\psi^*\bm{\nabla}^2\psi-\psi\bm{\nabla}^2\psi^*)
$$
这里使用的空间恒等式是：
$$
\psi^*\bm{\nabla}^2\psi-\psi\bm{\nabla}^2\psi^*=\bm{\nabla}\cdot(\psi^*\bm{\nabla}\psi-\psi\bm{\nabla}\psi^*)
...
因此薛定谔方程给出：
$$
\partial_t\rho_{\text{S}}+\bm{\nabla}\cdot\bm{j}_{\text{S}}=0
$$
其中 $\rho_{\text{S}}=\psi^*\psi=|\psi|^2$，而空间流为：
$$
\bm{j}_{\text{S}}=\frac{1}{2M\mathrm{i}}(\psi^*\bm{\nabla}\psi-\psi\bm{\nabla}\psi^*)
$$
$\rho_{\text{S}}$ 在每一点都非负。
...
$$
整理以后得到 $J^\mu=\mathrm{i}[\phi^*\pdv*{\phi}{x_\mu}-\phi\pdv*{\phi^*}{x_\mu}]=j^\mu$。
...
$$
重复刚才的乘积法则证明可得 $\pdv*{j^\mu(\phi_1,\phi_2)}{x^\mu}=0$。
在固定时间切片 $\Sigma_t$ 上定义 Klein–Gordon 半双线性型：
$$
\langle\phi_1,\phi_2\rangle_{\text{KG}}=\mathrm{i}\int[\phi_1^*\partial_t\phi_2-(\partial_t\phi_1^*)\phi_2]\dd[3]{x}
$$
半双线性表示它对第一个输入反线性，对第二个输入线性。
直接取复共轭还可以验证共轭对称性 $\langle\phi_1,\phi_2\rangle_{\text{KG}}^*=\langle\phi_2,\phi_1\rangle_{\text{KG}}$。


这个量的守恒需要边界条件。
对时间求导并使用混合流的连续性方程：
$$
\dv{}{t}\langle\phi_1,\phi_2\rangle_{\text{KG}}=-\oint_{\partial\Sigma_t}\bm{j}(\phi_1,\phi_2)\cdot\dd{\bm{S}}
$$
若无穷远通量消失，内积与所选时间无关。
更协变地，对任意类空间柯西超曲面 $\Sigma$ 可以写成：
$$
\langle\phi_1,\phi_2\rangle_{\text{KG}}=\int_\Sigma j^\mu(\phi_1,\phi_2)\dd{\Sigma_\mu}
$$
$\dd{\Sigma_\mu}$ 是带未来指向法向量的超曲面元。
四维高斯定理和 $\pdv*{j^\mu}{x^\mu}=0$ 说明，只要两张柯西面之间的无穷远边界没有通量，这个积分不依赖选择哪一张柯西面。
...
$$
对正频率模有 $\pdv*{u_{\bm{k}}}{t}=-\mathrm{i}\omega_{\bm{k}}u_{\bm{k}}$。
代入 Klein–Gordon 型：
$$
\langle u_{\bm{k}},u_{\bm{k}'}\rangle_{\text{KG}}=\frac{\omega_{\bm{k}}+\omega_{\bm{k}'}}{2\sqrt{\omega_{\bm{k}}\omega_{\bm{k}'}}V}\mathrm{e}^{\mathrm{i}(\omega_{\bm{k}}-\omega_{\bm{k}'})t}\int_V\mathrm{e}^{\mathrm{i}(\bm{k}'-\bm{k})\cdot\bm{x}}\dd[3]{x}
$$
空间积分给出 $V\delta_{\bm{k}\bm{k}'}$。
克罗内克符号又迫使 $\omega_{\bm{k}}=\omega_{\bm{k}'}$，所以前面的系数化为 $1$：
$$
\langle u_{\bm{k}},u_{\bm{k}'}\rangle_{\text{KG}}=\delta_{\bm{k}\bm{k}'}
$$
...
$$
\langle v_{\bm{k}},v_{\bm{k}'}\rangle_{\text{KG}}=-\delta_{\bm{k}\bm{k}'}
$$
正频率模与负频率模相互正交：
$$
\langle u_{\bm{k}},v_{\bm{k}'}\rangle_{\text{KG}}=0
$$
最后一个等式可以直接计算。
...
利用刚才三条正交关系：
$$
\langle\phi,\phi\rangle_{\text{KG}}=\sum_{\bm{k}}(|A_{\bm{k}}|^2-|B_{\bm{k}}|^2)
$$
它是守恒的，却不是正定的。
...
负范数与负能量来自同一个频率符号。
让时间平移生成元 $\mathrm{i}\pdv*{}{t}$ 作用在两个模上：
$$
...
$$
\langle\phi,\phi\rangle_{\text{KG}}=\sum_{\bm{k}}|A_{\bm{k}}|^2
$$
...
实标量场使这一点更加明显。
若 $\phi^*=\phi$，上述流恒等于零 $j^\mu=\mathrm{i}[\phi\pdv*{\phi}{x_\mu}-\phi\pdv*{\phi}{x_\mu}]=0$。
...
归一化的洛伦兹不变性也需要准确表述。
不变量是超曲面积分 $\langle\phi_1,\phi_2\rangle_{\text{KG}}$，或等价的质量壳测度 $\dd[3]{p}/(2E_{\bm{p}})$ 所定义的配对。
...
哈密顿量为：
$$
H=\frac{1}{2}\int[\pi^2+(\bm{\nabla}\phi)^2+m^2\phi^2]\dd[3]{y}
$$
海森堡方程写成 $\pdv*{O}{t}=\mathrm{i}[H,O]$。
...
$$
\partial_t\pi(t,\bm{x})=\frac{\mathrm{i}}{2}\int[(\bm{\nabla}_{\bm{y}}\phi)^2+m^2\phi^2,\pi(t,\bm{x})]\dd[3]{y}
$$
质量项中的对易子为：
$$
...
乘积对易恒等式给出：
$$
[(\bm{\nabla}_{\bm{y}}\phi)^2,\pi(\bm{x})]=2\mathrm{i}\bm{\nabla}_{\bm{y}}\phi(\bm{y})\cdot\bm{\nabla}_{\bm{y}}\delta^{(3)}(\bm{y}-\bm{x})
$$
所以梯度项为：
$$
\frac{\mathrm{i}}{2}\int[(\bm{\nabla}_{\bm{y}}\phi)^2,\pi(\bm{x})]\dd[3]{y}=-\int\bm{\nabla}_{\bm{y}}\phi(\bm{y})\cdot\bm{\nabla}_{\bm{y}}\delta^{(3)}(\bm{y}-\bm{x})\dd[3]{y}=\bm{\nabla}^2\phi(\bm{x})
$$
