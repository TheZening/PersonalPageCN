...
把场拆成两部分：
$$
A^\mu(x)=A_{(+)}^\mu(x)+A_{(-)}^\mu(x)
$$
正频率部分定义为：
$$
A_{(+)}^\mu(x)=\sum_{\lambda=0}^3\int\frac{\varepsilon_\lambda^\mu(\bm{k})\hat{a}_\lambda(\bm{k})\mathrm{e}^{-\mathrm{i}k\cdot x}}{(2\pi)^3\sqrt{2\omega_{\bm{k}}}}\dd[3]{\bm{k}}
$$
负频率部分为：
$$
A_{(-)}^\mu(x)=\sum_{\lambda=0}^3\int\frac{\varepsilon_\lambda^{\mu*}(\bm{k})\hat{a}_\lambda^\dagger(\bm{k})\mathrm{e}^{+\mathrm{i}k\cdot x}}{(2\pi)^3\sqrt{2\omega_{\bm{k}}}}\dd[3]{\bm{k}}
$$
其中四维动量满足：
$$
...
正频率部分的厄米共轭为：
$$
[A_{(+)}^\mu]^\dagger(x)=\sum_{\lambda=0}^3\int\frac{\varepsilon_\lambda^{\mu*}(\bm{k})\hat{a}_\lambda^\dagger(\bm{k})\mathrm{e}^{+\mathrm{i}k\cdot x}}{(2\pi)^3\sqrt{2\omega_{\bm{k}}}}\dd[3]{\bm{k}}
$$
所以：
$$
[A_{(+)}^\mu]^\dagger(x)=A_{(-)}^\mu(x)
$$
这里的下标 $(+)$ 和 $(-)$ 指频率符号，不是电荷正负，也不是产生与湮灭算符的符号。
...
把 $A_\nu$ 作为正则坐标，其共轭动量为：
$$
\Pi^\nu=\pdv{\mathcal{L}}{(\partial_0A_\nu)}
$$
由简化拉格朗日密度得到：
$$
\Pi^\nu=-\partial^0A^\nu=-\dot A^\nu
$$
与未经规范固定的麦克斯韦理论不同，现在 $\Pi^0=-\pdv*{A^0}{t}$ 不再恒为零。
...
s_1=s_2=s_3=1
$$
为了真正从场对易关系推出模代数，而不是先猜模对易子的符号，在固定时刻对场展开作傅里叶投影。
利用伪正交关系可反演出：
$$
\hat{a}_\lambda(\bm{k})=\eta_{\lambda\lambda}\int\frac{\varepsilon_{\lambda\mu}^*(\bm{k})[\omega_{\bm{k}}A^\mu(t,\bm{x})+\mathrm{i}\dot A^\mu(t,\bm{x})]\mathrm{e}^{+\mathrm{i}k\cdot x}}{\sqrt{2\omega_{\bm{k}}}}\dd[3]{\bm{x}}
$$
这里固定偏振标签 $\lambda$，不对它求和。
其中指数中的 $k\cdot x=\omega_{\bm{k}}t-\bm{k}\cdot\bm{x}$。
组合 $\omega_{\bm{k}}A^\mu+\mathrm{i}\pdv*{A^\mu}{t}$ 使负频率部分相消，而外面的 $\eta_{\lambda\lambda}$ 抵消偏振投影产生的同一个号差。
取厄米共轭得到：
$$
\hat{a}_{\lambda'}^\dagger(\bm{q})=\eta_{\lambda'\lambda'}\int\frac{\varepsilon_{\lambda'\nu}(\bm{q})[\omega_{\bm{q}}A^\nu(t,\bm{y})-\mathrm{i}\dot A^\nu(t,\bm{y})]\mathrm{e}^{-\mathrm{i}q\cdot y}}{\sqrt{2\omega_{\bm{q}}}}\dd[3]{\bm{y}}
$$
由三条等时场对易关系，两个方括号中组合的对易子为：
$$
[\omega_{\bm{k}}A^\mu(t,\bm{x})+\mathrm{i}\dot A^\mu(t,\bm{x}),\omega_{\bm{q}}A^\nu(t,\bm{y})-\mathrm{i}\dot A^\nu(t,\bm{y})]=-(\omega_{\bm{k}}+\omega_{\bm{q}})\eta^{\mu\nu}\delta^{(3)}(\bm{x}-\bm{y})
$$
把它代入两条反演公式，先用空间狄拉克分布完成 $\bm{y}$ 积分，再用：
$$
\int\mathrm{e}^{-\mathrm{i}(\bm{k}-\bm{q})\cdot\bm{x}}\dd[3]{\bm{x}}=(2\pi)^3\delta^{(3)}(\bm{k}-\bm{q})
$$
动量狄拉克分布令 $\omega_{\bm{k}}=\omega_{\bm{q}}$，而偏振收缩给出 $\varepsilon_{\lambda\mu}^*\varepsilon_{\lambda'}^\mu=\eta_{\lambda\lambda'}$。
于是：
$$
[\hat{a}_\lambda(\bm{k}),\hat{a}_{\lambda'}^\dagger(\bm{q})]=-\eta_{\lambda\lambda}(2\pi)^3\delta_{\lambda\lambda'}\delta^{(3)}(\bm{k}-\bm{q})=s_\lambda(2\pi)^3\delta_{\lambda\lambda'}\delta^{(3)}(\bm{k}-\bm{q})
$$
时间型模的负号至此直接来自场的正则对易关系。
同样把反演公式代入另外两类场对易关系，可得两个湮灭算符或两个产生算符之间的对易子为零：
$$
...
$$
I_1^{\mu\nu}=\frac{\mathrm{i}}{2}\sum_\lambda s_\lambda\int\frac{\varepsilon_\lambda^\mu(\bm{k})\varepsilon_\lambda^{\nu*}(\bm{k})\mathrm{e}^{+\mathrm{i}\bm{k}\cdot(\bm{x}-\bm{y})}}{(2\pi)^3}\dd[3]{\bm{k}}
$$
...
再使用傅里叶表示：
$$
\delta^{(3)}(\bm{x}-\bm{y})=\int\frac{\mathrm{e}^{+\mathrm{i}\bm{k}\cdot(\bm{x}-\bm{y})}}{(2\pi)^3}\dd[3]{\bm{k}}
$$
最终得到：
$$
[A^\mu(t,\bm{x}),\dot A^\nu(t,\bm{y})]=-\mathrm{i}\eta^{\mu\nu}\delta^{(3)}(\bm{x}-\bm{y})
