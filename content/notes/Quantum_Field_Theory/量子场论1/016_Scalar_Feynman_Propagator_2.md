...
上一篇已经从时间序定义得到传播子的三维动量表示，并证明它满足归一化为 $-\mathrm{i}\delta^{(4)}$ 的非齐次 Klein–Gordon 方程。
现在要把两个时间分支统一成一个四维动量积分。
完成这一步的关键不是形式上多写一个 $k^0$ 积分，而是说明积分路径如何绕过 $k^0=\pm\omega_{\bm{k}}$ 两个极点。
本篇逐项计算两个时间方向的围道积分，推出 $\mathrm{i}\epsilon$ 处方，再解释离壳动量、其他边界条件的核和“虚粒子”语言的准确边界。
...
目标是证明这两项恰好等于一个四维积分：
$$
D_F(z)=\int\frac{\mathrm{i}\mathrm{e}^{-\mathrm{i}kz}}{k^2-m^2+\mathrm{i}\epsilon}\frac{\dd[4]{k}}{(2\pi)^4}
$$
...
$$
I_{\bm{k}}(z^0)=\int_{-\infty}^{+\infty}\frac{\mathrm{i}\mathrm{e}^{-\mathrm{i}k^0z^0}}{(k^0)^2-\omega_{\bm{k}}^2+\mathrm{i}\epsilon}\frac{\dd{k^0}}{2\pi}
$$
四维传播子就是：
$$
D_F(z)=\int\mathrm{e}^{\mathrm{i}\bm{k}\cdot\bm{z}}I_{\bm{k}}(z^0)\frac{\dd[3]{k}}{(2\pi)^3}
$$
...
留数定理因此给出负号：
$$
\int_{-\infty}^{+\infty}f(k^0)\dd{k^0}=-2\pi\mathrm{i}\sum_{\text{下半平面}}\operatorname{Res}f
$$
...
代回三维积分：
$$
D_F(z)=\int\frac{\mathrm{e}^{-\mathrm{i}\omega_{\bm{k}}z^0+\mathrm{i}\bm{k}\cdot\bm{z}}}{2(2\pi)^3\omega_{\bm{k}}}\dd[3]{k} \quad z^0>0
$$
这正是 $\theta(z^0)D_+(z)$ 分支。
...
路径方向为逆时针，所以：
$$
\int_{-\infty}^{+\infty}f(k^0)\dd{k^0}=2\pi\mathrm{i}\sum_{\text{上半平面}}\operatorname{Res}f
$$
...
所以：
$$
(\Box+m^2)D_F(z)=-\int\frac{\mathrm{i}(k^2-m^2)\mathrm{e}^{-\mathrm{i}kz}}{k^2-m^2+\mathrm{i}\epsilon}\frac{\dd[4]{k}}{(2\pi)^4}
$$
取分布极限 $\epsilon\to0^+$ 后，分式趋于 $1$：
$$
(\Box+m^2)D_F(z)=-\mathrm{i}\int\mathrm{e}^{-\mathrm{i}kz}\frac{\dd[4]{k}}{(2\pi)^4}
$$
使用四维傅里叶表示的德尔塔分布：
$$
\delta^{(4)}(z)=\int\mathrm{e}^{-\mathrm{i}kz}\frac{\dd[4]{k}}{(2\pi)^4}
$$
立即恢复：
$$
...
采用傅里叶变换约定：
$$
D_F(z)=\int\widetilde{D}_F(k)\mathrm{e}^{-\mathrm{i}kz}\frac{\dd[4]{k}}{(2\pi)^4}
$$
位置空间公式直接给出动量空间传播子：
$$
...
$$
严格格林核 $G_F=\mathrm{i}D_F=-\Delta_F$ 的动量空间形式相应为：
$$
\widetilde{G}_F(k)=-\frac{1}{k^2-m^2+\mathrm{i}\epsilon}
$$
这些写法必须与各自的位置空间定义配套，不能从一本教材取左边记号，再从另一本教材取右边分子。
...
严格说，$G_F=\mathrm{i}D_F$ 才是这个符号的分布逆，而 $D_F$ 是量子场论惯例下相差固定因子的时间序传播子。
质量壳正是符号失去可逆性的零点集合。
...
同一个微分算符可以有不同边界条件的逆核，差别完全体现在极点放置上。
...
$$
$G_R=\mathrm{i}D_R$ 与 $G_A=\mathrm{i}D_A$ 才是源归一化为 $+\delta^{(4)}$ 的严格格林核。
$D_R$ 只在源事件之后响应，$D_A$ 只在源事件之前非零。
Feynman 传播子则按正负频率安排时间边界，它不是经典意义上的纯推迟响应函数。


怀特曼函数与这些非齐次核也不能混为一谈。
$D_+$ 和 $D_-$ 各自满足齐次 Klein–Gordon 方程，没有四维德尔塔源。
...
严格得到的是自由真空中两个场算符的时间序关联，它与 Klein–Gordon 算符在 Feynman 边界条件下的分布逆只相差固定因子。
