...
在这个过程中还会看见两个重要事实：齐次方程其实是 $F=\mathrm{d}A$ 的几何恒等式，而非齐次方程之间的依赖关系正是规范对称性的诺特第二定理。
...
因此 $F^{\mu\nu}$ 的矩阵形式为：
$$
F^{\mu\nu}=\mqty[0&-E^1&-E^2&-E^3\\E^1&0&-B^3&B^2\\E^2&B^3&0&-B^1\\E^3&-B^2&B^1&0]
$$
...
取四维列维–奇维塔张量的约定为 $\epsilon^{0123}=+1$，再定义对偶场强：
$$
...
$$
这就是比安基恒等式的几何版本。
它说明齐次麦克斯韦方程不是作用量额外施加的运动方程，而是场强由势产生后自动满足的可积条件。
...
换成三维场就是：
$$
\mathcal{L}_{\text{EM}}=\frac{1}{2}(\bm{E}^2-\bm{B}^2)
$$
微分形式写法还揭示度规在哪里进入理论。
...
作用量变分为：
$$
\delta S=\int\left[-\frac{1}{4}\delta(F_{\mu\nu}F^{\mu\nu})-j^\nu\delta A_\nu\right]\dd[4]{x}
$$
度规不参与变分，所以乘积法则给出：
$$
\delta(F_{\mu\nu}F^{\mu\nu})=2F^{\mu\nu}\delta F_{\mu\nu}
...
$$
这正是两条有源麦克斯韦方程的协变形式。
另外两条齐次方程已经由 $F=\mathrm{d}A$ 自动保证，因此四条方程在作用量语言中本来就不处于完全对称的地位。
...
其中：
$$
\Box=\partial_\mu\partial^\mu=\partial_t^2-\bm{\nabla}^2
$$
若选择洛伦茨规范条件：
$$
\partial_\mu A^\mu=0
...
含外源时，耦合项的变化为：
$$
\delta_\chi S_{\text{int}}=-\int j^\mu\partial_\mu\chi\dd[4]{x}
$$
分部积分得到：
$$
\delta_\chi S_{\text{int}}=\int(\partial_\mu j^\mu)\chi\dd[4]{x}-\int\partial_\mu(j^\mu\chi)\dd[4]{x}
$$
...
$$
它描述电磁场穿过法向为第 $i$ 个方向的曲面所传递的第 $j$ 个动量分量通量。
