...
更几何地说，固定时刻的场构型空间记为 $\mathcal{C}$，共轭动量是作用在构型变分上的对偶变量，所以经典相空间局部具有余切丛 $T^*\mathcal{C}$ 的结构。
一对 $(\phi,\pi)$ 是这个无穷维余切丛中的一个点，而泊松括号来自相空间上的辛结构。
辛结构是一种非退化反对称双线性型；它把一个泛函的微分转换成相空间中的哈密顿向量场，从而决定这个泛函生成怎样的正则变换。


场的可观测量一般是函数的函数，数学上称为泛函。
例如哈密顿量：
$$
H[\phi,\pi]=\int\frac{1}{2}[\pi^2+(\bm{\nabla}\phi)^2+m^2\phi^2]\dd[3]{x}
$$
...
它由一阶变分定义：
$$
\delta F=\int\left[\fdv{F}{\phi(\bm{x})}\delta\phi(\bm{x})+\fdv{F}{\pi(\bm{x})}\delta\pi(\bm{x})\right]\dd[3]{x}
$$
这里 $\fdv*{F}{\phi(\bm{x})}$ 表示只在 $\bm{x}$ 附近改变场时，泛函 $F$ 的一阶响应密度。
...
所以：
$$
\fdv{\phi(\bm{y})}{\phi(\bm{x})}=\delta^{(3)}(\bm{x}-\bm{y})
$$
...
$$
\{F,G\}_{\text{P}}=\int\left(\fdv{F}{\phi(\bm{x})}\fdv{G}{\pi(\bm{x})}-\fdv{F}{\pi(\bm{x})}\fdv{G}{\phi(\bm{x})}\right)\dd[3]{x}
$$
...
这里的狄拉克分布同时承担两个角色：它既是位置连续指标上的单位核，也表达不同空间点的正则坐标彼此独立。
所以等时泊松括号不是在每个点各自放置一个孤立公式，而是无穷维相空间辛结构在位置表象中的分布核。


正则量子化首先要求基本正则变量实现与经典泊松括号相同的代数结构。
在自然单位制 $\hbar=1$ 下，对应关系为：
$$
\{\phi(t,\bm{x}),\pi(t,\bm{y})\}_{\text{P}}=\delta^{(3)}(\bm{x}-\bm{y}) \quad \to \quad [\phi(t,\bm{x}),\pi(t,\bm{y})]=\mathrm{i}\delta^{(3)}(\bm{x}-\bm{y})
$$
因此实标量场的基本等时对易关系为：
$$
...
常见的简写 $\{F,G\}_{\text{P}}\to[F,G]/\mathrm{i}$ 对基本正则变量以及关于它们至多线性的可观测量是可靠的。
对一般非线性泛函，经典乘法是可交换的，量子算符乘法却不可交换，因而必须另行指定算符排序。
更严格地说，不存在一个把全部经典泊松代数无歧义地映到量子对易代数并同时保存所有代数关系的量子化映射，这就是 Groenewold–van Hove 阻碍。
所以本篇只把基本等时对易关系作为量子化输入，后面的复合算符则逐一规定排序或正规化。
...
允许动量为 $\bm{k}=2\pi\bm{n}/L$，其中 $\bm{n}\in\mathbb{Z}^3$，并定义：
$$
...
$$
\hat{a}_{\bm{k}}=\langle u_{\bm{k}},\phi\rangle_{\text{KG}} \quad \hat{a}_{\bm{k}}^{\dagger}=-\langle u_{\bm{k}}^*,\phi\rangle_{\text{KG}}
$$
其中 $u_{\bm{k}}=\mathrm{e}^{-\mathrm{i}kx}/\sqrt{2\omega_{\bm{k}}V}$。
...
取无穷体积极限时使用替换：
$$
\frac{1}{V}\sum_{\bm{k}}F(\bm{k})\to\int F(\bm{k})\frac{\dd[3]{k}}{(2\pi)^3} \quad V\delta_{\bm{k}\bm{p}}\to(2\pi)^3\delta^{(3)}(\bm{k}-\bm{p})
$$
为了保持模展开整体不变，离散与连续模算符的对应为：
$$
\hat{a}(\bm{k})=\sqrt{\frac{V}{(2\pi)^3}}\hat{a}_{\bm{k}}
$$
把它和 $\delta_{\bm{k}\bm{p}}\to(2\pi)^3\delta^{(3)}(\bm{k}-\bm{p})/V$ 一起代入，就得到连续谱的狄拉克 $\delta$ 分布对易关系。
本系列选择把 $\sqrt{(2\pi)^3}$ 吸收到连续模展开的归一化因子中：
$$
\phi(x)=\int\frac{\hat{a}(\bm{k})\mathrm{e}^{-\mathrm{i}kx}+\hat{a}^{\dagger}(\bm{k})\mathrm{e}^{\mathrm{i}kx}}{\sqrt{2(2\pi)^3\omega_{\bm{k}}}}\dd[3]{k}
$$
在这个约定下：
$$
...
$$
其他两类对易子为零。
另一种常见约定采用 $\dd[3]{k}/[(2\pi)^3 2\omega_{\bm{k}}]$ 作为积分测度，那时对易子右边会变成 $(2\pi)^3 2\omega_{\bm{k}}\delta^{(3)}(\bm{k}-\bm{p})$。
...
$$
所有不配对的等时对易子都为零。
注意 $\phi$ 的正则动量是 $\pdv*{\phi^{\dagger}}{t}$，而不是 $\pdv*{\phi}{t}$。
