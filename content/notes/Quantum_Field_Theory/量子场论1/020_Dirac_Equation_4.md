...
Dirac 方程及其共轭方程分别为：
$$
\mathrm{i}\gamma^\mu\partial_\mu\psi=m\psi,\mathrm{i}(\partial_\mu\bar{\psi})\gamma^\mu=-m\bar{\psi}
$$
定义四维流：
$$
...
$$
\dv{}{t}\int j^0\dd[3]{x}=-\int\bm{\nabla}\cdot\bm{j}\dd[3]{x}=-\oint_{S_\infty}\bm{j}\cdot\dd{\bm{S}}=0
$$
所以总概率：
$$
\int\psi^\dagger\psi\dd[3]{x}
$$
不随时间变化。
...
$$
(\psi_1,\psi_2)=\int\psi_1^\dagger(t,\bm{x})\psi_2(t,\bm{x})\dd[3]{x}
$$
对交叉流 $j_{12}^\mu=\bar{\psi}_1\gamma^\mu\psi_2$ 重复刚才的推导，可得 $\partial_\mu j_{12}^\mu=0$，所以这个内积也与时间无关。
更协变地，可以在任意类空超曲面 $\Sigma$ 上写成：
$$
(\psi_1,\psi_2)_\Sigma=\int_\Sigma\bar{\psi}_1\gamma^\mu\psi_2\dd{\Sigma_\mu}
$$
类空超曲面可以形象地理解为“一整张空间切片”：沿切片的任意无穷小切向位移都是类空的。
若进一步选取类空的 Cauchy（柯西）曲面，它还要求每条完整的类时因果轨迹都与这张切片相交一次，因此这张切片能够承载一套完整的初始数据。
$\dd{\Sigma_\mu}$ 是这张切片的有向三维面积元，它的方向沿切片的类时法线。
在 $t$ 为常数的切片上，$\dd{\Sigma_\mu}=(\dd[3]{x},0,0,0)$，上式就回到普通等时内积。
...
$$
由于 $\alpha^i$、$\beta$ 和三维动量算符 $\bm{p}=-\mathrm{i}\bm{\nabla}$ 都是厄米的，适当边界条件下 $H$ 也是厄米的。
这从算符角度再次说明时间演化保持内积。
...
在标准 Dirac 表示中定义 $\Sigma^i=\mqty[\sigma^i&0\\0&\sigma^i]$ 与 $S^i=1/2\Sigma^i$。
...
$$
\bm{S}^2=\frac{1}{4}\sum_i(\Sigma^i)^2=\frac{3}{4}\mathbb{1}_4
$$
与角动量本征值 $s(s+1)$ 比较可得 $s=1/2$。
因此，四分量并不表示自旋为 $3/2$；正能量和负能量分支各自都携带两个自旋为 $1/2$ 的态。
...
若二分量旋量满足：
$$
(\bm{\sigma}\cdot\bm{n}_{\bm{p}})\xi_\lambda=2\lambda\xi_\lambda
$$
其中 $\bm{n}_{\bm{p}}=\bm{p}/|\bm{p}|$，那么 $\lambda=\pm1/2$。
...
对于严格无质量粒子，没有惯性观察者能够超过它，所以螺旋度在正规正时洛伦兹变换下保持不变。
空间反演会反转动量而不把自旋这个轴矢量反转，因此会翻转螺旋度；“洛伦兹不变”在这里不能把宇称变换也包含进去。
对无质量的正能量自由解，螺旋度与手征性可以建立直接对应；有质量时两者不能混用。


从群表示角度看，固定一个质量壳动量以后，仍保持这个动量不变的洛伦兹变换组成它的稳定子群，粒子的自旋标签正来自这个稳定子群在对应解空间纤维上的表示。
有质量粒子可以回到静止动量，稳定子群表现为普通空间转动，所以二维纤维承载自旋 $1/2$ 表示。
螺旋度则是在选定运动方向后把这个纤维表示按动量轴上的转动生成元对角化所得的标签；它不是旋量丛中新添的一根纤维。
...
$$
\mathrm{i}\pdv{\psi}{t}=\mqty[m&\bm{\sigma}\cdot\bm{p}\\\bm{\sigma}\cdot\bm{p}&-m]\psi
$$
时间导数作用在静止能量相位和缓慢变化的旋量上：
$$
\mathrm{i}\pdv{\psi}{t}=\mathrm{e}^{-\mathrm{i}mt}\left[m\mqty[\phi\\\chi]+\mathrm{i}\mqty[\partial_t\phi\\\partial_t\chi]\right]
$$
比较上下两个二分量，得到耦合方程：
$$
...
$$
\mathrm{i}\partial_t\phi\simeq\frac{\bm{p}^2}{2m}\phi=-\frac{\bm{\nabla}^2}{2m}\phi
$$
