...
从左侧乘以 $S^{-1}$，得到：
$$
[\mathrm{i}S^{-1}\gamma^\mu S(\Lambda^{-1})^\nu{}_\mu\partial_\nu-m]\psi=0
$$
...
更准确地说，旋量表示来自 $\mathrm{Spin}_0(1,3)$，它双覆盖与恒等变换连通的正规正时洛伦兹群 $\mathrm{SO}_0(1,3)$。
$\mathrm{SO}_0(1,3)$ 描述保持时空取向与时间方向并与恒等元连通的洛伦兹变换，而 $\mathrm{Spin}_0(1,3)$ 可以理解为在这些变换之上额外保留旋量符号的一层群结构。
...
$\mathrm{Spin}_0(1,3)$ 在这些标架上作用，再通过狄拉克表示构造出旋量丛；$S(\Lambda)$ 描述的正是局部自旋标架变化时旋量纤维分量怎样改变。
...
无穷小旋量变换为：
$$
S(\Lambda)=\mathbb{1}_4-\frac{\mathrm{i}}{4}\omega_{\mu\nu}\sigma^{\mu\nu}+O(\omega^2)
$$
...
$$
由于 $(\bm{n}\cdot\bm{\Sigma})^2=\mathbb{1}_4$，指数可以直接求出：
$$
S_R(\theta)=\mathbb{1}_4\cos\frac{\theta}{2}-\mathrm{i}\bm{n}\cdot\bm{\Sigma}\sin\frac{\theta}{2}
$$
因此 $S_R(2\pi)=-\mathbb{1}_4$，而 $S_R(4\pi)=\mathbb{1}_4$。
...
$$
因为 $(\bm{n}\cdot\bm{\alpha})^2=\mathbb{1}_4$，所以：
$$
S_B(\zeta)=\mathbb{1}_4\cosh\frac{\zeta}{2}+\bm{n}\cdot\bm{\alpha}\sinh\frac{\zeta}{2}
$$
...
对 Dirac 方程取厄米共轭：
$$
[(\mathrm{i}\gamma^\mu\partial_\mu-m)\psi]^\dagger=0
$$
导数作用方向反转后得到：
$$
...
$$
由克利福德代数可以推出 $(\gamma^5)^\dagger=\gamma^5$、$(\gamma^5)^2=\mathbb{1}_4$ 与 $\{\gamma^5,\gamma^\mu\}=0$。


例如，把 $\gamma^\mu$ 从 $\gamma^5$ 的四个 $\gamma$ 矩阵中逐次移到另一侧时，总会产生奇数次反交换，所以得到负号。
$\gamma^5$ 的本征值为 $\pm1$，相应投影算符为 $P_R=(\mathbb{1}_4+\gamma^5)/2$ 与 $P_L=(\mathbb{1}_4-\gamma^5)/2$。


它们满足 $P_R^2=P_R$、$P_L^2=P_L$、$P_RP_L=0$ 与 $P_R+P_L=\mathbb{1}_4$，因而把 Dirac 旋量分成右手征与左手征两部分。
...
对 $\psi$ 变分时，两个偏导数分别为：
$$
\pdv{\mathcal{L}}{\psi}=-m\bar{\psi}
$$
以及：
$$
\pdv{\mathcal{L}}{(\partial_\mu\psi)}=\mathrm{i}\bar{\psi}\gamma^\mu
$$
因此欧拉–拉格朗日方程给出：
$$
