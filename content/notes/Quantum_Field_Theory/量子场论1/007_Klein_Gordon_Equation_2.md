...
这里的“质量壳”是四维动量空间中的三维子流形，它由一个方程 $p^2=m^2$ 从四维空间中切出来。
每个质量壳动量 $p$ 上方都可以放置满足场方程的模振幅；对标量场，单个正频率模的纤维只有一维，所以平面波系数只是一个复数。
后面的旋量场和光子场会在同一质量壳上方带有二维自旋或偏振空间，届时这些随 $p$ 变化的空间才会形成非平凡得多的解空间丛。
...
色散关系还给出波包的群速度：
$$
\bm{v}_{\text{g}}=\bm{\nabla}_{\bm{k}}\omega_{\bm{k}}=\frac{\bm{k}}{\sqrt{\bm{k}^2+m^2}}
$$
对 $m>0$，其大小小于 $1$。
...
傅里叶展开可以理解为在经典解空间中选择一套连续基底，系数函数则是同一个解在这套基底下的坐标。
洛伦兹不变测度不是额外装饰，而是质量壳这个子流形上与洛伦兹群作用相容的积分规则；它保证换惯性系以后，对连续模坐标的求和仍表示同一个几何配对。
