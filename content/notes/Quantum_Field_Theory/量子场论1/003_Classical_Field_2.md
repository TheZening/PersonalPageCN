...
$$
\sum_{i=1}^N aF(q_i,\Delta q_i/a)\to\int F(\phi(x),\partial_x\phi(x))\dd{x}
$$
...
本文只处理一阶局域理论，所以允许的局域数据是 $x$，$\phi_a(x)$ 和 $\pdv*{\phi_a(x)}{x^\mu}$。
这并不意味着不同点彼此没有作用。
...
固定 $x$ 后，若 $\phi$ 与 $\widetilde{\phi}$ 满足 $\phi(x)=\widetilde{\phi}(x)$ 且 $\pdv*{\phi(x)}{x^\mu}=\pdv*{\widetilde{\phi}(x)}{x^\mu}$，就把它们归入同一类。
...
对应到物理上，$\phi_a(x)$ 是局域场值，$\pdv*{\phi_a(x)}{x^0}$ 描述它在该事件附近的时间变化率，$\pdv*{\phi_a(x)}{x^i}$ 描述空间方向上的局域梯度。
...
使用乘积求导法则：
$$
\pdv{\mathcal{L}_{\text{kin}}}{(\partial_\lambda\phi)}=\frac{1}{2}\eta^{\mu\nu}\left[\delta_\mu^\lambda\partial_\nu\phi+\partial_\mu\phi\delta_\nu^\lambda\right]
$$
第一项收缩 $\mu$ 指标，第二项收缩 $\nu$ 指标：
$$
\pdv{\mathcal{L}_{\text{kin}}}{(\partial_\lambda\phi)}=\frac{1}{2}\eta^{\lambda\nu}\partial_\nu\phi+\frac{1}{2}\eta^{\mu\lambda}\partial_\mu\phi
$$
度规对称，两项相等，所以：
$$
...
$$
这个运动方程对时间是二阶的。
因此在一个初始时刻 $t_0$，一般需要同时给定 $\phi(t_0,\bm{x})$ 与 $\pdv*{\phi}{t}(t_0,\bm{x})$ 才能确定后续演化。
前者是初始构型，后者是初始速度。
...
$$
再对 $\phi$ 变分。
对应的两个偏导数为 $\pdv*{\mathcal{L}}{\phi}=-m^2\phi^*$ 与 $\pdv*{\mathcal{L}}{(\pdv*{\phi}{x^\mu})}=\eta^{\mu\nu}\pdv*{\phi^*}{x^\nu}$。
所以得到共轭方程：
$$
(\Box+m^2)\phi^*=0
$$
...
更具体地说，固定时刻后每个空间点的场值 $\phi_a(\bm{x})$ 都像一个广义坐标，场速度 $\pdv*{\phi_a}{t}(\bm{x})$ 是这个坐标的速度。
...
这个勒让德变换要求能够用 $\phi_a$ 和 $\pi_a$ 解出速度 $\pdv*{\phi_a}{t}$。
判断局部可逆性的对象是速度黑塞矩阵：
$$
W_{ab}=\pdv{\mathcal{L}}{(\partial_t\phi_a)}{(\partial_t\phi_b)}
$$
...
$$
速度黑塞矩阵等于 $1$，因而勒让德变换可逆，并有 $\pdv*{\phi}{t}=\pi$。
