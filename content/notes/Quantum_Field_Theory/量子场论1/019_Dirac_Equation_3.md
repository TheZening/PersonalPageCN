...
若这里只研究线性方程的解，展开系数可以暂时取普通复数；若把它作为待量子化的经典费米场或路径积分变量，则应采用下一篇介绍的格拉斯曼奇系数。
...
$$
由于 $\pdv*{\mathrm{e}^{-\mathrm{i}p\cdot x}}{x^\mu}=-\mathrm{i}p_\mu\mathrm{e}^{-\mathrm{i}p\cdot x}$，代入方程后得到：
$$
(\slashed{p}-m)u(p)=0
$$
再考虑负频率平面波：
...
$$
这时 $\pdv*{\mathrm{e}^{+\mathrm{i}p\cdot x}}{x^\mu}=+\mathrm{i}p_\mu\mathrm{e}^{+\mathrm{i}p\cdot x}$，所以：
$$
(\slashed{p}+m)v(p)=0
$$
...
因此有两个二分量方程：
$$
(E-m)\phi-(\bm{\sigma}\cdot\bm{p})\chi=0,(\bm{\sigma}\cdot\bm{p})\phi-(E+m)\chi=0
$$
第二式可以直接解出下分量：
$$
...
$$
\xi_r^\dagger\xi_s=\delta_{rs},\sum_{s=1}^2\xi_s\xi_s^\dagger=\mathbb{1}_{2\times2}
$$
选择相对论性归一化后，正频率旋量为：
$$
...
$$
(E+m)\varphi-(\bm{\sigma}\cdot\bm{p})\eta=0,(\bm{\sigma}\cdot\bm{p})\varphi-(E-m)\eta=0
$$
第一式给出：
$$
...
$$
\eta_r^\dagger\eta_s=\delta_{rs},\sum_{s=1}^2\eta_s\eta_s^\dagger=\mathbb{1}_{2\times2}
$$
负频率旋量可以选为：
$$
...
$$
u_r^\dagger(p)u_s(p)=(E+m)\xi_r^\dagger\left[\mathbb{1}_2+\frac{(\bm{\sigma}\cdot\bm{p})^2}{(E+m)^2}\right]\xi_s
$$
...
$$
\bar{u}_s(p)=\sqrt{E+m}\mqty[\xi_s^\dagger&-\xi_s^\dagger\frac{\bm{\sigma}\cdot\bm{p}}{E+m}]
$$
因此：
$$
\bar{u}_r(p)u_s(p)=(E+m)\xi_r^\dagger\left[1-\frac{\bm{p}^2}{(E+m)^2}\right]\xi_s
$$
括号中的因子为：
$$
...
直接代入可得：
$$
\bar{u}_r(p)v_s(p)=(E+m)\left[\xi_r^\dagger\frac{\bm{\sigma}\cdot\bm{p}}{E+m}\eta_s-\xi_r^\dagger\frac{\bm{\sigma}\cdot\bm{p}}{E+m}\eta_s\right]=0
$$
同理：
$$
\bar{v}_r(p)u_s(p)=0
$$
接下来推导自旋求和公式。
对 $u$ 旋量求和，并使用 $\sum_s\xi_s\xi_s^\dagger=\mathbb{1}_{2\times2}$：
$$
\sum_su_s(p)\bar{u}_s(p)=(E+m)\mqty[\mathbb{1}&-\frac{\bm{\sigma}\cdot\bm{p}}{E+m}\\\frac{\bm{\sigma}\cdot\bm{p}}{E+m}&-\frac{\bm{p}^2}{(E+m)^2}\mathbb{1}]
$$
右下分块可以利用 $\bm{p}^2=(E-m)(E+m)$ 化简为 $-(E-m)$，所以：
$$
\sum_su_s(p)\bar{u}_s(p)=\mqty[E+m&-\bm{\sigma}\cdot\bm{p}\\\bm{\sigma}\cdot\bm{p}&-E+m]=\slashed{p}+m
$$
同样地，对 $v$ 旋量求和：
$$
\sum_sv_s(p)\bar{v}_s(p)=(E+m)\mqty[\frac{\bm{p}^2}{(E+m)^2}\mathbb{1}&-\frac{\bm{\sigma}\cdot\bm{p}}{E+m}\\\frac{\bm{\sigma}\cdot\bm{p}}{E+m}&-\mathbb{1}]
$$
左上分块为 $E-m$，因此：
$$
\sum_sv_s(p)\bar{v}_s(p)=\mqty[E-m&-\bm{\sigma}\cdot\bm{p}\\\bm{\sigma}\cdot\bm{p}&-E-m]=\slashed{p}-m
$$
最终，正频率解的完备关系为：
$$
...
\Lambda_-(p)=\frac{-\slashed{p}+m}{2m}
$$
这些含 $1/(2m)$ 的投影算符只适用于 $m>0$；无质量极限应改用能量、螺旋度或手征投影，不能在这里直接令 $m\to0$。
...
相应的 Dirac 内积约定为：
$$
\bar{u}_ru_s=\delta_{rs},\bar{v}_rv_s=-\delta_{rs}
$$
...
$$
\psi(x)=\sum_s\int\frac{[b_s(\bm{p})u_s(p)\mathrm{e}^{-\mathrm{i}p\cdot x}+d_s^*(\bm{p})v_s(p)\mathrm{e}^{+\mathrm{i}p\cdot x}]}{(2\pi)^3\sqrt{2E_{\bm{p}}}}\dd[3]{p}
$$
在只求复线性 Dirac 方程通解时，$b_s$ 与 $d_s^*$ 可以看成两族解的复系数。
在经典费米场的分级作用量或费米路径积分中，它们相应地是格拉斯曼奇系数。
