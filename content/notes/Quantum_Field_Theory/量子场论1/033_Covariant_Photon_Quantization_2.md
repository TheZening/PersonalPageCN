...
上一篇得到时间型模的对易关系 $[\hat{a}_0,\hat{a}_0^\dagger]=-1$。
如果仍把 $\hat{a}_0^\dagger|0\rangle$ 当作普通希尔伯特空间中的一粒子态，它的范数平方就会是负数。
...
$$
\hat{a}_\lambda(\bm{k})|0\rangle=0
$$

对固定的离散动量和偏振，暂时省略这些标签，并记：

$$
[\hat{a},\hat{a}^\dagger]=s
$$
...
$$
\langle1|1\rangle=\langle0|\hat{a}\hat{a}^\dagger|0\rangle
$$

把产生算符移到左边：

$$
\hat{a}\hat{a}^\dagger=\hat{a}^\dagger\hat{a}+s
$$
...
$$
\langle n|n\rangle=ns\langle n-1|n-1\rangle
$$

从 $\langle0|0\rangle=1$ 出发：

$$
\langle n|n\rangle=s^n n!
$$
...
真空满足 $\hat{N}|0\rangle=0$。
利用第一条对易关系：

$$
\hat{N}(\hat{a}^\dagger)^n|0\rangle=(\hat{a}^\dagger)^n(\hat{N}+n)|0\rangle
$$
...
$$
|n\rangle_s=\frac{1}{\sqrt{n!}}(\hat{a}^\dagger)^n|0\rangle
$$

其内积为：

$$
{}_s\langle n|n\rangle_s=s^n
$$

产生算符作用为：

$$
\hat{a}^\dagger|n\rangle_s=\sqrt{n+1}|n+1\rangle_s
$$

湮灭算符作用时保留号差：

$$
\hat{a}|n\rangle_s=s\sqrt{n}|n-1\rangle_s
$$
...
$$
\hat{N}_{\mathrm{aux}}=\sum_{\lambda=0}^3\int\frac{\mathrm{d}^3k}{(2\pi)^3}\hat{N}_\lambda(\bm{k})
$$
...
把模展开代入 $H=\int\mathrm{d}^3x\mathcal{H}$。
空间积分使用：

$$
\int\mathrm{d}^3x\mathrm{e}^{+\mathrm{i}(\bm{k}-\bm{q})\cdot\bm{x}}=(2\pi)^3\delta^{(3)}(\bm{k}-\bm{q})
$$
...
$$
\hat{H}=\frac{1}{2}\sum_{\bm{k},\lambda}\omega_{\bm{k}}s_\lambda[\hat{a}_\lambda^\dagger(\bm{k})\hat{a}_\lambda(\bm{k})+\hat{a}_\lambda(\bm{k})\hat{a}_\lambda^\dagger(\bm{k})]
$$
...
$$
\hat{H}=\sum_{\bm{k},\lambda}\omega_{\bm{k}}\left[\hat{N}_\lambda(\bm{k})+\frac{1}{2}\right]
$$
...
$$
[:\hat{H}:,\hat{a}_\lambda^\dagger(\bm{k})]=\omega_{\bm{k}}\hat{a}_\lambda^\dagger(\bm{k})
$$

所以：

$$
:\hat{H}:\hat{a}_\lambda^\dagger(\bm{k})|0\rangle=\omega_{\bm{k}}\hat{a}_\lambda^\dagger(\bm{k})|0\rangle
$$
...
$$
[:\widehat{\bm{P}}:,\hat{a}_\lambda^\dagger(\bm{k})]=\bm{k}\hat{a}_\lambda^\dagger(\bm{k})
$$
