...
$$
利用 $T^2=\mathbb{1}$ 可以直接验证:
$$
P_{\text{sym}}^2=P_{\text{sym}},\quad P_{\text{alt}}^2=P_{\text{alt}}.
$$
它们还满足:
$$
P_{\text{sym}}P_{\text{alt}}=0,
$$
以及:
$$
P_{\text{sym}}+P_{\text{alt}}=\mathbb{1}.
$$


所以两个投影的像空间给出直和分解:
$$
V\otimes V=\operatorname{Im}P_{\text{sym}}\oplus\operatorname{Im}P_{\text{alt}}.
$$
定义 $V$ 的**对称平方**与**反对称平方**为:
$$
\operatorname{Sym}^2(V)=\operatorname{Im}P_{\text{sym}},\quad \Lambda^2(V)=\operatorname{Im}P_{\text{alt}}.
$$
...
等价地:
$$
\operatorname{Sym}^2(V)=\{\xi\in V\otimes V\mid T\xi=\xi\},
$$
并且:
$$
\Lambda^2(V)=\{\xi\in V\otimes V\mid T\xi=-\xi\}.
$$
...
张量平方上的对角作用为:
$$
R(g)=\rho(g)\otimes\rho(g).
$$
交换因子与在两个因子上同时施加相同的群作用互不干扰, 所以:
$$
TR(g)=R(g)T.
$$
...
因此两个投影的像空间都在群作用下保持不变, 从而:
$$
\boxed{V\otimes V=\operatorname{Sym}^2(V)\oplus\Lambda^2(V)}
$$
是 $G$-表示的直和分解.
...
那么 $A$ 保持 $\operatorname{Im}P$ 与 $\ker P$, 并且:
$$
\operatorname{tr}(A|_{\operatorname{Im}P})=\operatorname{tr}(PA).
$$
...
首先, 张量积特征标公式给出:
$$
\operatorname{tr}R(g)=\operatorname{tr}(\rho(g)\otimes\rho(g))=\chi_V(g)^2.
$$
...
令 $A=\rho(g)$, 得到:
$$
\operatorname{tr}(TR(g))=\operatorname{tr}(\rho(g)^2)=\chi_V(g^2).
$$
...
因此:
$$
\boxed{\chi_{\operatorname{Sym}^2(V)}(g)=\frac{1}{2}(\chi_V(g)^2+\chi_V(g^2))}.
$$
...
因此:
$$
\boxed{\chi_{\Lambda^2(V)}(g)=\frac{1}{2}(\chi_V(g)^2-\chi_V(g^2))}.
$$
...
把两个特征标公式相加, 得到:
$$
\chi_{\operatorname{Sym}^2(V)}(g)+\chi_{\Lambda^2(V)}(g)=\chi_V(g)^2.
$$
...
代入两个特征标公式得到:
$$
\boxed{\dim\operatorname{Sym}^2(V)=\frac{d(d+1)}{2}},
$$
以及:
$$
\boxed{\dim\Lambda^2(V)=\frac{d(d-1)}{2}}.
$$
...
它在对称平方中的重数为:
$$
m_{\alpha,\text{sym}}=\langle\chi_{\alpha},\chi_{\operatorname{Sym}^2(V)}\rangle_G.
$$
它在反对称平方中的重数为:
$$
m_{\alpha,\text{alt}}=\langle\chi_{\alpha},\chi_{\Lambda^2(V)}\rangle_G.
$$
...
双线性形式空间为:
$$
\operatorname{Bil}(V)=V_{\text{dual}}\otimes V_{\text{dual}}.
$$
它也分解为对称与反对称两部分:
$$
V_{\text{dual}}\otimes V_{\text{dual}}=\operatorname{Sym}^2(V_{\text{dual}})\oplus\Lambda^2(V_{\text{dual}}).
$$
因此不变对称双线性形式空间满足:
$$
\operatorname{Bil}_{\text{sym}}(V)\cong(\operatorname{Sym}^2(V_{\text{dual}}))_{\text{inv}},
$$
而不变反对称双线性形式空间满足:
$$
\operatorname{Bil}_{\text{alt}}(V)\cong(\Lambda^2(V_{\text{dual}}))_{\text{inv}}.
$$
