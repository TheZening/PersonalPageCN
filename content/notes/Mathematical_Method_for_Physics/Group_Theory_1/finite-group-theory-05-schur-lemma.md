...
最后说明复数域条件的作用, 并解释怎样用自交织映射判断表示是否不可约.
...
复数域的作用非常具体: 每个有限维复线性算符至少有一个复特征值.
在实数域上, 特征多项式可能没有实根, 因而前一节从 $T$ 选取特征值的证明会中断.
这不是证明技巧不够, 而是结论本身在实数域上可能失效.


> **阅读提示:** 现在只需记住复数域保证 $T$ 有特征值; 实表示的更细分类将在第 19–21 篇单独处理, 此处不需要提前掌握.


不可约性同样不能删去.
若表示已经分成几个不变分量, 就可以在不同分量上乘以不同常数, 仍然与全部群作用对易.
Schur 引理只在单个不可约分量内部迫使这些常数相同.
...
这条判据可以平直地理解为: 如果一个表示还能被拆开, 投影到其中一块的算符就会与群作用对易; 如果所有对易算符都只能整体乘同一个数, 就没有这样的分块可选.
